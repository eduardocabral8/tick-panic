...
describe('User', () => {
  ...
  it('should assign a unique id', () => {
    const u1 = new User('alice', 'h1', 'player');
    const u2 = new User('bob', 'h2', 'player');
  ...
  it('should record createdAt date', () => {
    ...
    const user = new User('alice', 'h', 'player');
  ...
  it('should throw if username is empty', () => {
    expect(() => new User('', 'hash', 'player')).toThrow('Username is required');
  });
  ...
  it('should throw if username exceeds 50 characters', () => {
    ...
    expect(() => new User(longName, 'hash', 'player')).toThrow('Username must be 50 characters or less');
  });

  it('should throw if passwordHash is empty', () => {
    expect(() => new User('alice', '', 'player')).toThrow('Password hash is required');
  });
  ...
  it('should only allow admin or player role', () => {
    expect(() => new User('alice', 'hash', 'guest' as unknown as UserRole)).toThrow('Role must be admin or player');
  });
});
